...
public abstract class PathNavigator {
   ...
   protected void pathFollow() {
      ...
      // Forge: fix MC-94054
      if (Math.abs(this.entity.func_226277_ct_() - (vec3d1.x + ((this.entity.getWidth() + 1) / 2D))) < (double)this.maxDistanceToWaypoint && Math.abs(this.entity.func_226281_cx_() - (vec3d1.z + ((this.entity.getWidth() + 1) / 2D))) < (double)this.maxDistanceToWaypoint && Math.abs(this.entity.func_226278_cu_() - vec3d1.y) < 1.0D) {
         this.currentPath.setCurrentPathIndex(this.currentPath.getCurrentPathIndex() + 1);
      }
